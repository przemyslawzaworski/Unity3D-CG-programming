...
Shader "Effect Transition Extended"
{
	Subshader
	{
		Pass
		...
		Pass
		{
			...
			float4 pixel_shader (custom_type ps ) : SV_TARGET
			{
				...
				return lerp(tex2D(_bufferA,uv),raymarch(ro,rd),min(max(sign(uv.x-n),0.0)*(1.0-abs(sign(t)))+max(sign(1.0-uv.x - n),0.0)*(1.0-abs(sign(t-1.0))),1.0));
			}
